  ...
  return (
    <>
      ...
      <DrawerItem name="Kubernetes Version">
        {object.spec?.kubernetes?.version ?? "-"}
      </DrawerItem>

      {/* Spec: Control Plane */}
      {object.spec?.controlPlane && (
        <>
          <DrawerItem name="Control Plane Replicas">
            {object.spec.controlPlane.deployment?.replicas ?? "-"}
          </DrawerItem>
          {object.spec.controlPlane.service?.serviceType && (
            <DrawerItem name="Control Plane Service Type">
              {object.spec.controlPlane.service.serviceType}
            </DrawerItem>
          )}
        </>
      ...
      {object.spec?.dataStore && (
        <>
          <DrawerItem name="Data Store Name">
            {object.spec.dataStore.name ?? "-"}
          </DrawerItem>
          {object.spec.dataStore.externalName && (
            <DrawerItem name="Data Store External Name">
              {object.spec.dataStore.externalName}
            </DrawerItem>
          )}
        </>
      ...
      {object.status?.kubernetesResources && (
        <>
          <DrawerItem name="Kubernetes Version (Status)">
            <Badge
              label="Version"
              tooltip={object.status.kubernetesResources.version?.status}
            >
      ...
      {object.status?.storage && (
        <>
          <DrawerItem name="Storage Driver">
            {object.status.storage.driver ?? "-"}
          </DrawerItem>
          <DrawerItem name="Data Store Name (Status)">
            {object.status.storage.dataStoreName ?? "-"}
          </DrawerItem>
        </>
      )}
      ...
      {object.status?.addons?.konnectivity && (
        <DrawerItem name="Konnectivity Status">
          <Badge
            label="Status"
            tooltip={object.status.addons.konnectivity.status}
          >
